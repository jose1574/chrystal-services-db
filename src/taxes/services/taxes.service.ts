...
export class TaxesService {
  ...
  async findOneByCode(code: string): Promise<TaxesDto> {
    ...
    if (!tax) {
      throw new NotFoundException(`tax con el codigo ${code} no se encuentra.`);
    }
    return tax;
  }
  ...
  async update(id: string, changes: UpdateTaxesDto): Promise<UpdateTaxesDto> {
    try {
      ...
      const updateTax = this.taxesRepository.save(merge);
  ...
  async delete(id: string): Promise<TaxesDto> {
    const taxDelete = await this.findOneByCode(id);
    if (!taxDelete) {
      throw new NotFoundException(`taxes con ID ${id} no encontrado`);
    }
    await this.taxesRepository.remove(taxDelete);
    return taxDelete;
  }
}
